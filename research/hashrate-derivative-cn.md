...
合约发生交割时，Market智能合约将会向合约头寸持有人发送WETH。每份合约多头将收到 ( _EMI_ - _Floor_ ) 个WETH，每份合约空头将收到 ( _Cap_ - _EMI_ ) 个 WETH。
...
比特币算力收益指数（ _BMI_ ，BTC Mining Index ）表示以区块高度 _H_的挖矿难度，在 _Hashrate_（单位Hash/s） 算力挖矿2016个块获得的BTC数学期望。_Difficulty<sub>i</sub>_ 表示区块高度 _i_ 的挖矿难度, _Coinbase<sub>i</sub>_ 表示区块高度 _i_ 的挖矿奖励,  _TargetBlockTime_ 表示比特币预期出块时间。 _BMI_ 由以下公式计算：
...
例，在2019年5月1日时，BMI指数为526, 市场中比特币挖矿收益合约的成交价情况如下：
