...
按照Market Protocol的规则，每份比特币算力收益合约都会设置指数的有效范围，用 _Cap_ 表示指数上限，用 _Floor_ 表示指数下限。当指数达到上下限的时候会触发合约交割。

在发行合约的时候，为了减少对保证金的需求、提高杠杆，会尽量设置较小的指数范围，另一方面，为了防止因指数达到上下限而触发合约提前交割，会设置相对安全的指数范围。
